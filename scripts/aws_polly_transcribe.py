    ...
    with file_path.open("r") as file:
        ...
        for row in reader:
            term = VocabularyTerm(
                ...
                ipa=row.get("IPA"),
                display_as=row.get("DisplayAs")
                or row["Phrase"],  # Use Phrase if DisplayAs is empty
            )
...
def create_processed_csv(vocab_terms: List[VocabularyTerm], bucket: str) -> str:
    ...
    writer = csv.DictWriter(
        csv_buffer, fieldnames=["Phrase", "SoundsLike", "IPA", "DisplayAs"]
    )
    writer.writeheader()
...
async def create_custom_vocabulary(
    vocabulary_name: str, vocab_terms: List[VocabularyTerm]
) -> None:
    client = get_client("transcribe")

    phrases = []
    for term in vocab_terms:
        phrase_entry = {"Phrase": term.phrase, "DisplayAs": term.display_as}
        if term.sounds_like:
    ...
    try:
        ...
        click.echo(
            f"Custom vocabulary creation initiated: {response['VocabularyName']}"
        )
    ...
    try:
        ...
        while True:
            ...
            if "Messages" in response:
                ...
                if message["detail"]["TranscriptionJobStatus"] in [
                    "COMPLETED",
                    "FAILED",
                ]:
                    sqs.delete_message(
        ...
        if message["detail"]["TranscriptionJobStatus"] == "COMPLETED":
            ...
            response = s3.get_object(
                Bucket=output_bucket, Key=f"{job_name}-output.json"
            )
            transcript = json.loads(response["Body"].read().decode("utf-8"))
            send_sms_notification(
                phone_number, "Transcription job completed successfully."
            )
...
@click.option(
    ...
    default="resources/aws-transcribe-custom-vocabulary.csv",
...
@click.option(
    "--input-bucket",
    type=str,
    default="aif-c01-input",
    help="Name of the S3 bucket for input files",
)
@click.option(
    "--output-bucket",
    type=str,
    default="aif-c01-output",
...
async def main(
    ...
    if confirm_upload:
        click.echo(
            f"Input bucket '{input_bucket}' and output bucket '{output_bucket}' are ready for use."
        )
        return
    ...
    try:
        ...
        click.echo(
            f"Processed CSV file uploaded to S3: s3://{output_bucket}/{processed_csv_file}"
        )
        ...
        transcript = await transcribe_audio(
            audio_file.name, input_bucket, output_bucket, vocabulary_name, phone_number
        )
